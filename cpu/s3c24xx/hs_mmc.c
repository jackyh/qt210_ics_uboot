...
#define s3c_hsmmc_readl(x)	readl(ELFIN_HSMMC_BASE + (x))
#define s3c_hsmmc_readw(x)	readw(ELFIN_HSMMC_BASE + (x))
#define s3c_hsmmc_readb(x)	readb(ELFIN_HSMMC_BASE + (x))

#define s3c_hsmmc_writel(v,x)	writel((v),(ELFIN_HSMMC_BASE + (x)))
#define s3c_hsmmc_writew(v,x)	writew((v),(ELFIN_HSMMC_BASE + (x)))
#define s3c_hsmmc_writeb(v,x)	writeb((v),(ELFIN_HSMMC_BASE + (x)))
...
void hsmmc_reset (void)
{
	...
	s3c_hsmmc_writeb(0x1, HM_SWRST);

	timeout = 100;

	while (s3c_hsmmc_readb(HM_SWRST) & 0x1) {
		if (timeout == 0) {
			printf("hsmmc reset failed\n");
			return;
		}
		timeout--;
		udelay(1000);
...
void hsmmc_set_gpio (void)
{
	...
	reg = readl(GPECON) & ~(0x003ffc00);
	writel(reg | 0x002aa800, GPECON);

	reg = readl(GPLCON) & ~(0x000f00ff);
	writel(reg | 0x000a00aa, GPLCON);
...
static int set_sd_ocr (void)
{
	...
	issue_command(SD_SEND_IF_COND, 0x1aa, 0, 0x75);
	//issue_command(SD_SEND_IF_COND, 0x1aa, 0, MMC_RSP_R7);
